...
const access_token = isEmpty(secureStorage.get(APP_AUTHORIZE_USER_KEY)) ? "V345o52ghvdcgh765dZ" : JSON?.parse(secureStorage.get(APP_AUTHORIZE_USER_KEY));
const commandApi = createApiRequest(API_URL);
const Authorization = {
    login: (data) =>
        commandApi.post("/customer-login", data, {
            headers: {
    ...
    getUserDetails:()=>
        commandApi.get("/customer-account-details")
};
